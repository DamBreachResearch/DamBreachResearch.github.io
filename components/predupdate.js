let hwForm, vwForm, hdForm, hbForm, wForm, recalForm, erodForm, modeForm, typeForm;
let qEqnSelect, qPredResult, qUpperResult, warningOutput, qDesc;
...
function update_pred_result() {
    ...
    if (q){
        ...
        warningOutput.innerHTML = "Warning: Peak flow prediction is not available. Ensure H<sub>w</sub> and V<sub>w</sub> have valid values.";
    }
    ...
    if (tf){
        ...
        warningOutput.innerHTML = "Warning: Time to failure prediction is not available. Ensure H<sub>w</sub> and V<sub>w</sub> have valid values.";
    }
    // Get the upper bound based on the selected equation's performance
...
function draw_qp_chart() {
    ...
    let qFunc = recalForm.checked ? q_eqn_map.get(qEqn).func_re : q_eqn_map.get(qEqn).func;
    let getQ = (v) => qFunc({
    ...
    let estimate_data = [{x: vw, y: 0}, {x: vw, y: getQ(vw)}, {x: minV, y: getQ(vw)}];
